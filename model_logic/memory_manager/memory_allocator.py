...
class MemoryAllocator:
    ...
    def alloc_contiguous(self, need_size):
        ...
        can_used_loc = self.indexes[0 : self.tot_size - need_size + 1][
            loc_sums == need_size
        ]
    ...
    def alloc_grid(self, need_grid, grid_size):
        ...
        mask = grid_size - 1
        loc_use = (
            (self.indexes[: self.tot_size - grid_size + 1] & mask) == 0
        ) & loc_use
        ...
        select_index = torch.empty(
            (grid_size, need_grid), dtype=torch.int32, device="cuda"
        )
    ...
    def alloc_prefix(self, need_size):
        ...
        if need_size > self.can_use_mem_size_prefix:
            raise Exception(
                f"warn no enough pool space: need_size {need_size} left_size {self.can_use_mem_size_prefix}"
            )
    ...
    def alloc_contiguous_prefix(self, need_size):
        ...
        loc_sums = (
            ...
            - self._mem_cum_sum[0 : self.cache_size - need_size + 1]
            + self.mem_state[0 : self.cache_size - need_size + 1]
        )
        can_used_loc = self.indexes[0 : self.cache_size - need_size + 1][
            loc_sums == need_size
        ]
        if can_used_loc.shape[0] == 0:
    ...
    def alloc_suffix(self, need_size):
        ...
        if need_size > self.can_use_mem_size_suffix:
            raise Exception(
                f"warn no enough pool space: need_size {need_size} left_size {self.can_use_mem_size_suffix}"
            )
            return None
    ...
    def reset_all_pool(self):
        ...
        self._mem_cum_sum = torch.empty(
            (self.tot_size,), dtype=torch.int32, device="cuda"
        )
        ...
        self.key_buffer = [
            torch.empty(
                (self.tot_size, self.head_num, self.head_dim),
                dtype=self.dtype,
                device="cuda",
            )
